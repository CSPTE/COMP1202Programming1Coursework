...
public class StudentTeam extends Team {
	...
	void move(Character member, Team enemyTeam) throws Exception {
		if (member.getHP() > 0) {
			...
			if (member instanceof AEStudent) {
				aeStudent(member, enemyTeam, target, deadMember, deadFriend);
			}
	...
	/**
	 * This method selects which ability a AEStudent should use
	 ...
	 * @param deadFriend - The dead friend that will be revived and healed
	 ...
	 */
	private void aeStudent(Character member, Team enemyTeam, Character target, int deadMember, Character deadFriend)
		...
		if(member.getHP() > (member.getMaxHP()/2)) {
			if(((Student) member).getKP() == ((Student) member).getMaxKP()) {
				if (deadMember > 0) {
					...
					System.out.println(member.getName() + " Used Space Invasion on enemy team");
				}
			} else {
